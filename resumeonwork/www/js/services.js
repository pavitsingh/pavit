...
.service('PostService', ['$rootScope', '$http', '$q', 'WORDPRESS_API_URL', function ($rootScope, $http, $q, WORDPRESS_API_URL){
	...
	this.getRecentPosts = function(page) {
		...
		$http.jsonp('http://www.blog.resumeon.com/api/get_recent_posts/' +
		'?page='+ page +
	...
	this.getPost = function(postId) {
		...
		$http.jsonp('http://www.blog.resumeon.com/api/get_post/' +
		'?post_id='+ postId +
		'&callback=JSON_CALLBACK')
		.success(function(data) {
			console.log(data);
...
.service('JobDetService', function() {

	//for the purpose of this example I will store user data on ionic local storage but you should save it on a database
	  var setJobDet = function(job_data) {
	    window.localStorage['jobdet'] = JSON.stringify(job_data);
	  };

	  var getJobDet = function(){
	    return JSON.parse(window.localStorage['jobdet'] || '{}');
	  };

	  var deleteJobDet = function(){
	  ...
	  return {
	    getJobDet: getJobDet,
	    setJobDet : setJobDet,
	    deleteJobDet : deleteJobDet
	  }
	})


.service('JobsearchService', function() {

	//for the purpose of this example I will store user data on ionic local storage but you should save it on a database
	  var setJobsearchDet = function(skill,location) {
	    window.localStorage['jobsearchdet'] = JSON.stringify(skill+"88991122"+location);
	  };

	  var getJobSearchDet = function(){
	    return JSON.parse(window.localStorage['jobsearchdet'] || '{}');
	  };

	  var deleteJobSearchDet = function(){
	      window.localStorage.clear();
	  };

	  return {
		  getJobSearchDet : getJobSearchDet,
		  setJobsearchDet : setJobsearchDet,
		  deleteJobSearchDet : deleteJobSearchDet
	  }
	})
